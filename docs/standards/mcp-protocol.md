...
**Mechanical check.** A repo whose source constructs a server (`FastMCP(...)`, `mcp.NewServer(...)`, `server.NewMCPServer(...)`, or `Server("name")` with a literal first argument) registers at least one tool the auditor can name. A bare `NewServer(...)` does not count — `httptest.NewServer` and `grpc.NewServer` are not tool surfaces — and neither does an accessor like `cfg.Server()`. Python registrations are found by AST: any decorator whose attribute is `tool` (`@mcp.tool`, `@server.tool`), and any decorator naming a module- or class-level factory that returns `x.tool(...)` applied to a function. Go registrations are found by pattern: `WithTools("name")`, `AddTool(...)` / `NewTool("name", ...)`, and a `Tool{Name: "name"}` composite literal, whose name is read from the literal's own fields. A repo that constructs no server — a library, a client — passes.
