...
- **`sources.py`** — source-text accessors that scrub before a check reads them. `combined_code_only_text` drops comments *and* string literals, for checks whose subject is written in code; `combined_code_text` drops comments and docstrings but keeps literals, for checks whose subject *is* a literal (a `transport="streamable-http"` argument). Reach for one of them when matching raw text would count a marker inside a Go raw string or a Python docstring as real code. `combined_source_text` is the unscrubbed read and strips nothing; several `PROTO-*` checks still use it deliberately, so a raw call site is not automatically a bug.
...
Evidence is filed as issues on this public repo about private target repos, so it names a thing (a rule subject, a marker, a short list of repo-relative paths capped at five) and never carries a captured source span or an absolute path. `test_evidence_contract.py` holds failing rules to a 400-character single-line bound, so a matcher that starts capturing source is caught where it is introduced. Know what that bound does not reach: it measures failure evidence against the `bad_*` fixtures, so it binds only the branches those fixtures exercise, and `n/a` reasons are never measured at all even though `report.py` publishes them through the same renderer. Keep a `NotApplicable` reason a constant. `test_evidence_bounds.py` covers the sharper case: where the offending value is itself the secret (a tool name holding a password or internal hostname), evidence publishes where it was declared, not the value. For the same reason an `error` Finding records the exception's type name and never its message — `audit.py` drops the message deliberately, since it routinely carries an absolute path.
