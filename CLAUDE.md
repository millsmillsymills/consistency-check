...
- **`sources.py`** — source-text accessors that scrub comments and string literals (`combined_code_only_text`, `combined_code_text`, `strip_go_comments`, …). A check that greps source must go through these: matching raw text is how a marker inside a Go raw string or a Python docstring gets counted as real code.
...
3. Set `min_stage` (and `applies_to_archetype`, for deploy rules) to match the map in the standards doc; `tests/test_min_stage_map.py` pins the two together.
